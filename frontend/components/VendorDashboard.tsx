...
export const VendorDashboard: React.FC<VendorDashboardProps> = ({ setView }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FD] font-sans text-[#111111] flex overflow-hidden selection:bg-[#c3f235] selection:text-[#111111]">
      ...
      <aside className="w-72 bg-white flex flex-col border-r border-gray-100 flex-shrink-0 z-20">
          ...
          <div className="p-6 pb-2">
              <div className="flex items-center gap-2 mb-8 cursor-pointer" onClick={() => setView('LANDING')}>
                  <div className="w-8 h-8 bg-[#c3f235] text-white rounded-full flex items-center justify-center font-anton text-sm">L</div>
          ...
          <div className="px-6 flex-1 overflow-y-auto no-scrollbar">
              ...
              <nav className="space-y-1">
                  {menuItems.map((item) => (
                      ...
                      >
                          <item.icon size={18} strokeWidth={2} className={`${activeTab === item.id ? 'text-[#c3f235]' : 'text-gray-400 group-hover:text-[#111111]'}`} />
                          {item.label}
                      </button>
                  ))}
              </nav>
          ...
          <div className="p-6">
              <div className="bg-[#111111] rounded-[24px] p-6 text-center text-white relative overflow-hidden group">
                   <div className="absolute top-0 right-0 w-24 h-24 bg-[#c3f235] rounded-full blur-[40px] opacity-20 group-hover:opacity-40 transition-opacity"></div>
                   <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4 backdrop-blur-sm">
                       <Gem size={18} className="text-[#c3f235]" />
                   </div>
                   ...
                   <button className="w-full py-3 bg-[#c3f235] rounded-[14px] text-[10px] font-bold uppercase tracking-widest hover:bg-white hover:text-[#111111] transition-colors">
                       Upgrade Now
                   </button>
              </div>
          </div>
