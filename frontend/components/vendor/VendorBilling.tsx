...
export const VendorBilling: React.FC = () => {
    ...
    const StatusBar = () => (
        <div className="flex items-center text-[10px] font-bold uppercase tracking-widest gap-2 bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
            ...
            <ChevronRight size={12} className="text-gray-300"/>
            <span className={docStatus === 'Paid' ? 'text-[#c9b52e]' : 'text-gray-400'}>Paid</span>
            <ChevronRight size={12} className="text-gray-300"/>
    ...
    const ActionButtons = ({ type }: { type: 'SO' | 'INV' | 'PO' | 'BILL' }) => (
        <div className="flex gap-2">
             {docStatus === 'Draft' && (
                 <button onClick={() => setDocStatus('Confirmed')} className="bg-[#111111] text-white px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-[#c9b52e]">Confirm</button>
             )}
             ...
             {type === 'PO' && docStatus === 'Confirmed' && (
              ...
              {type === 'INV' && docStatus === 'Confirmed' && (
                 <button onClick={() => setView('INVOICE_PAYMENT')} className="ml-4 border border-[#c9b52e] text-[#c9b52e] px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-[#c9b52e] hover:text-white transition-colors">Register Payment</button>
             )}
             {type === 'BILL' && docStatus === 'Confirmed' && (
                 <button onClick={() => setView('BILL_PAYMENT')} className="ml-4 border border-[#c9b52e] text-[#c9b52e] px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-[#c9b52e] hover:text-white transition-colors">Register Payment</button>
             )}
        </div>
    );
        ...
        <div className="p-8 h-full animate-[fadeIn_0.3s_ease-out] flex items-center justify-center">
            <div className="bg-white w-full max-w-2xl p-10 rounded-xl shadow-2xl border border-gray-100">
                 <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4">
                     <h2 className="font-anton text-3xl uppercase tracking-wide text-[#c9b52e]">{isBill ? 'Bill Payment' : 'Invoice Payment'}</h2>
                 ...
                 <div className="flex justify-end gap-4 mt-10 pt-6 border-t border-gray-100">
                     ...
                     <button 
                        ...
                        className="px-6 py-2 bg-[#111111] text-white text-xs font-bold uppercase tracking-widest hover:bg-[#c9b52e] shadow-lg"
                    >
                        Confirm
                     </button>
                 </div>
        ...
        return (
             <div className="p-8 animate-[fadeIn_0.3s_ease-out]">
                ...
                <div className="bg-white p-8 border border-gray-200 shadow-sm rounded-lg relative">
                    ...
                    {docStatus === 'Paid' && (
                        <div className="absolute top-8 right-8 rotate-12 border-4 border-[#c9b52e] text-[#c9b52e] px-6 py-2 font-anton text-2xl uppercase tracking-widest opacity-80 select-none">
                            PAID
                        </div>
                    )}
                    ...
                    {/* Smart Button Link */}
                     ...
                     <div className="mt-4 text-right text-xs text-gray-500">
                         {docStatus === 'Paid' && <p className="text-[#c9b52e] font-bold mb-1">Paid on 12/12/2025: ₹600</p>}
